                         ...
                         total_ctrl_fsm_out_t *out)
{
    // ========== Output Logic ==========
    // ----- out_ready -----
    uint8_t in_state = (m->state == T_IDLE) ||
                       (m->state == T_CH02_IMG_IN) ||
                       (m->state == T_CH35_IMG_IN);
    ...
    out->is_ch35  = (m->state == T_CH35_IMG_IN) || (m->state == T_WAIT_MAC_35);
