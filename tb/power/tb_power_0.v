...
module tb_power_0;
    ...
    always @(posedge clk) begin
        cycle=cycle+1;
        if (!rst) begin
            check(fft_mag_valid === 0 || fft_mag_valid === 1, "Unknown Power valid");
            if (fft_mag_valid) begin
                check(supplied==128 && received<128, "Power frame length error");
                check(cycle == (received==0 ? final_input_cycle+2 : last_rx_cycle+1),
                      "Power capture timing error");
    ...
    task check_power_frame;
        ...
        begin
            ...
            if(peak_bin!=8 || peak_power==0) $fatal(1,"Expected positive peak bin 8, got %0d",peak_bin);
            if(!(received_power[8]>received_power[16] && received_power[8]>received_power[20]))
                $fatal(1,"BIT0 candidate is not dominant");
            $display("BIN POWER: P8=%0d P16=%0d P20=%0d mirror P120=%0d",
                received_power[8],received_power[16],received_power[20],received_power[120]);
            $display("TB_POWER_0 PASS: Fs=160000 Hz, N=128, peak bin=%0d, frequency=%0d Hz, expected symbol=BIT0 (0)",
    ...
    task calculate_expected;
        begin
            ar=$signed(reference_mem[a_addr][39:20]); ai=$signed(reference_mem[a_addr][19:0]);
            br=$signed(reference_mem[b_addr][39:20]); bi=$signed(reference_mem[b_addr][19:0]);
            wr=$signed(coefficients[w_addr][31:16]); wi=$signed(coefficients[w_addr][15:0]);
            tr=(br*wr-bi*wi) >>> 14;
            ti=(br*wi+bi*wr) >>> 14;
        ...
        begin
            ...
            begin : wait_for_result
                forever begin
                    ...
                    check(o_bf_out_valid === 0 || o_bf_out_valid === 1,"Unknown output valid");
            ...
            completed=completed+1; stage_count=stage_count+1;
        end
    endtask
