...
module tb_power_1;
    ...
    task check_power_frame;
        integer bin_id;
        begin
            csv_file=$fopen("power_1_comparison.csv","w");
            ...
            $fclose(csv_file);
            if(peak_bin!=16 || peak_power==0) $fatal(1,"Expected positive peak bin 16, got %0d",peak_bin);
            if(!(received_power[16]>received_power[8] && received_power[16]>received_power[20]))
                $fatal(1,"BIT1 (1) candidate is not dominant");
            $display("BIN POWER: P8=%0d P16=%0d P20=%0d mirror P112=%0d",
                received_power[8],received_power[16],received_power[20],received_power[112]);
            $display("TB_POWER_1 PASS: Fs=160000 Hz, N=128, peak bin=%0d, frequency=%0d Hz, expected symbol=BIT1 (1)",
